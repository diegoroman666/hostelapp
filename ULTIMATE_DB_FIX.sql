-- ULTIMATE DATABASE FIX (V4)
-- Run this in your Supabase SQL Editor to solve ALL "undefined" and Translation issues.

-- 1. Ensure the schema is correct
ALTER TABLE services ADD COLUMN IF NOT EXISTS key TEXT;
ALTER TABLE services ADD COLUMN IF NOT EXISTS category TEXT;

-- 2. Clear corrupted or incomplete data (Ensures symmetry)
TRUNCATE services;

-- 3. Populate ALL 17 services with their correct keys for translation
INSERT INTO services (id, name, key, description, price, is_active, image_url, category)
VALUES 
    -- MEALS (8 items - category 'meal')
    ...
    -- EXTRAS (9 items - category 'service')
...
-- 4. Final verification: Check room_prices table as well
-- (Ensures room keys 'key' column exists for getRoomName)
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='room_prices' AND column_name='key') THEN
        ALTER TABLE room_prices ADD COLUMN key TEXT;
    END IF;
END $$;

UPDATE room_prices SET key = 'constellation' WHERE room_type ILIKE '%constellation%';
UPDATE room_prices SET key = 'galaxy' WHERE room_type ILIKE '%galaxy%';
UPDATE room_prices SET key = 'nebula' WHERE room_type ILIKE '%nebula%';
UPDATE room_prices SET key = 'meteor' WHERE room_type ILIKE '%meteor%';
UPDATE room_prices SET key = 'comet' WHERE room_type ILIKE '%comet%';
UPDATE room_prices SET key = 'supernova' WHERE room_type ILIKE '%supernova%';
